  ...
  while (true) {
    for (int num : nums) {
      ...
      if ((loc_snapshot = appendLocation.fetch_add(1)) >= gs.size / sizeof(int)) {
        // No lock required, mremap will maintain same starting address
        gs.size += bufferSize * sizeof(int);
        if (ftruncate(gs.fd, gs.size) == -1) {
          std::cerr << "Failed to extend file.\n";
          return;
        }
        gs.addr = mremap(gs.addr, gs.size - bufferSize * sizeof(int), gs.size, MREMAP_MAYMOVE);
        if (gs.addr == MAP_FAILED) {
          std::cerr << "Failed to remap file.\n";
          return;
        }
      }
